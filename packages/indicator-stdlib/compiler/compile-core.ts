/**
 * Compilation logic shared by the Node and browser compilers.
 *
 * The only difference between them is which `asc` build is handed in, so everything else — the
 * virtual filesystem, the flags, the diagnostic mapping and the descriptor readback — lives here.
 * Keeping it in one place is what makes "the browser compiler produces the same bytes" testable
 * rather than a claim.
 */
...
/** The slice of `asc`'s API this module needs. Both builds satisfy it. */
export interface AscLike {
  createMemoryStream(): { toString(): string };
  main(args: string[], options: unknown): Promise<{ error?: Error | null }>;
}

export const ASC_ARGUMENTS = [
  ENTRY_FILENAME,
  "-O",
  "--noAssert",
  "--exportRuntime",
...
export async function compileWithAsc(
  asc: AscLike,
  source: string,
  ...
  const { error } = await asc.main(ASC_ARGUMENTS, {
    listFiles: () => [],
    readFile: (filename: string) => vfs.readFile(filename),
    reportDiagnostic: (diagnostic: AscDiagnostic) => {
      diagnostics.push(
  ...
  return {
    descriptor: await readDescriptor(wasm),
    diagnostics,
    ok: true,
    wasm,
  };
...
function userSourcePath(diagnostic: AscDiagnostic) {
...
/** Enough surface for a compiled module to link and describe itself. */
